...
### Addendum 2026-06-10 (avond) — externe validatie + preconditie-meting: sluiting nu ook extern onderbouwd

De overlay-sluiting is extern gevalideerd (deep-research, 24 primaire bronnen, 3-stemmen adversariële verificatie; research `docs/EXTERNAL-VALIDATION-EARLY-WARNING.md`): 11/12 interne kills zijn consistent met de literatuur (Cederburg 2020: 72/103 OOS-falen; Barroso & Detzel 2021: kosten). De ene goed-gerepliceerde uitzondering — Barroso & Santa-Clara 2015 eigen-variance-scaling van long-short momentum (Sharpe 0,53→0,97) — had een andere constructie dan ons gesneuvelde A0-vol-target en verdiende één pre-registered preconditie-meting: is de variance van óns boek voorspelbaar (B&S-C: AR(1) OOS R² 57,8% maandelijks)? **Uitslag: FAIL — 21d log OOS R² = 8,0% vs pre-registered lat 30%** (research `backtest/results/sweep/variance_predictability.json`, commit 72fb091). Het mechanisme is op dit boek afwezig; geen forward-paper test. Sentiment/on-chain/liquidaties/skew: nul geverifieerde claims → geen test gerechtvaardigd; alleen passief loggen zonder hypothese. **De overlay-sluiting staat daarmee intern (12 kills) én extern (literatuur + preconditie-fail).**
